...
- **雷曼第1–5卷第 2 页**：经全亮度墨迹检测确认为**全白空白页**（0.000% 墨迹，对照文字页约 4%），原无内容，已将占位符改为「空白页」标注，非内容丢失。
- **雷曼第5卷第 82 页**：低文本扫描页（LBSF 注资流程图），已补做 tesseract.js OCR 并嵌入正文。
- **雷曼第9卷附录**：无扫描页（文本层完整）。
...
- **转换质量校验（2026-08-03）**：对 33 个 PDF↔MD 全量核对——所有含实质文字的 PDF，其 MD 对有意义词覆盖度 **97–100%**（最低为众议院听证 97%，差 28 词属 OCR 噪声，可忽略）；扫描页（Vol7 24 + Vol8 83 + Vol5 p82 + 5388）已全部 OCR 回填，全仓库**零遗留占位符**；Vol1–5 第 2 页经墨迹检测为空白页（非丢失）。校验脚本：`validate_md.py`、`coverage_check.py`。
